...
from datamodels.optics.schema import DEFAULT_OUT_DIR, EXPORTED, GENERATOR_PYDANTIC, GeneratorMismatch, export_json_schemas, generator_matches, json_schemas, render
...
class TestJsonSchemaExport:
    def test_committed_schemas_match_a_fresh_export(self):
        if not generator_matches():
            pytest.skip(f"drift is checked under pydantic {GENERATOR_PYDANTIC} only (CI runs the pinned generator)")
        fresh = json_schemas()
    ...
    def test_schemas_record_their_generator_and_the_exporter_refuses_another(self, tmp_path, monkeypatch):
        for schema in json_schemas().values():
            assert schema["$comment"].startswith(f"generated by pydantic {GENERATOR_PYDANTIC}")
        for committed in SCHEMAS_DIR.glob("*.schema.json"):
            assert f"pydantic {GENERATOR_PYDANTIC}" in json.loads(committed.read_text())["$comment"]
        import pydantic
        monkeypatch.setattr(pydantic, "VERSION", "0.0.0")
        with pytest.raises(GeneratorMismatch):
