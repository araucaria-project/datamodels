...
from datamodels.optics.schema import EXPORTED, GENERATOR_PYDANTIC, GeneratorMismatch, export_json_schemas, json_schemas, package_version
...
class TestJsonSchemaExport:
    def test_schemas_carry_version_and_generator_and_the_exporter_refuses_another_pydantic(self, tmp_path, monkeypatch):
        release = package_version()
        for name, schema in json_schemas().items():
            assert schema["$id"] == f"https://araucaria-project.github.io/datamodels/optics/{release}/{name}.schema.json"
            assert f"pydantic {GENERATOR_PYDANTIC}" in schema["$comment"]
        import pydantic
